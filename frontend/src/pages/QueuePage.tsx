  ...
  return (
    ...
    >
      <div className="space-y-3">
        {items.length === 0 && <p className="text-sm text-slate-300">No queued deliveries yet.</p>}
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-slate-300">Campaign #{i.campaign_id}</p>
                </div>
                <span className="rounded-full bg-indigo-400/20 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-indigo-100">
                  {i.step}
                </span>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-300">
                ...
                <div className="rounded-lg border border-white/5 bg-white/5 px-3 py-2">
                  Status
                  <div className="text-sm font-semibold text-emerald-100">{i.status}</div>
                </div>
              </div>
            </motion.div>
          ))}
